...
entutor.utils.parseTime = function(t){
    ...
    if(tmp.length===2){
        ...
        seconds=parseFloat(tmp[1].replace(/,/,'.'));
        if (isNaN(seconds)) {
            seconds = 0;
        }
    ...
    if(tmp.length===3){
        ...
        seconds=parseFloat(tmp[2].replace(/,/,'.'));
        if (isNaN(seconds)) {
            seconds = 0;
        } 
...
entutor.inputs.card = function (parent, options) {
    ...
    this.showLock=false;
    this.onShowFinished=function(delay){
        return function(){
            if(self.onStart){
                self.onStart();
                self.onStart=false;
...
entutor.inputs.card.prototype.showChildren = function (_delay) {
    // console.log(this.id + ' showChildren ');
    var delay=parseInt(_delay);
    //console.log(this.id + ' showChildren '+delay);
...
entutor.inputs.html = function (parent, options) {
    ...
    this.duration = entutor.utils.parseTime(this.options.duration || 'none');
...
entutor.inputs.video = function (parent, options) {
    ...
    for(var i=0; i<this.subtitles.length; i++ ){
        this.subtitles[i].from=entutor.utils.parseTime(this.subtitles[i].from);
        this.subtitles[i].to=entutor.utils.parseTime(this.subtitles[i].to);
    }
...
entutor.inputs.dropzone.prototype.testSet=function(value){
    ...
    if(this.pattern.length!==value.length){
        result.score= 0;
        result.passed=false;
        return result;
    }
...
entutor.inputs.dropzone.prototype.draw = function () {
    ...
    this.dropzone = $('<span id="task' + this.id + 'dropzone" class="task-dropzone transition" style="min-width:' + (this.options.size || '4') + 'em;">&nbsp</span>');
...
entutor.inputs.slideshow = function (parent, options) {
    ...
    for(var i=0; i<this.slides.length; i++ ){
        this.slides[i].from=entutor.utils.parseTime(this.slides[i].from);
        this.slides[i].to=entutor.utils.parseTime(this.slides[i].to);
    }
...
entutor.html5recorder = function (parent, options) {
    ...
    this.autocheck=this.options.autocheck||false;
    this.autostart=this.options.autostart||false;
    this.duration = entutor.utils.parseTime(this.options.duration || 30); 
    this.taskPassScore=this.options.taskPassScore || 0.7;
    this.hideOnCorrect = this.options.hideOnCorrect? true : false;
